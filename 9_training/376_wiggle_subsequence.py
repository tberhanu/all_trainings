...
def wiggleMaxLength(nums):
    """Runtime: 36 ms, faster than 67.83% of Python3 online submissions for Wiggle Subsequence.
"""
    if len(nums) <= 1:
        return len(nums)
    i = 0
    ...
    prev = 999
    while i + 1 < len(nums):
        status = get_status(nums, i, i + 1)
        if status == 0:
            i = i + 1
        elif status != 0:
            counter = counter + 1 if status == -1 * prev or prev == 999 else counter
            prev = status
            i = i + 1

    return counter

def get_status(nums, i, j):
    if nums[i] == nums[j]:
        return 0
    else:
        return 1 if nums[i] > nums[j] else -1


print(wiggleMaxLength([3,3,3,2,5]))
print(wiggleMaxLength([1, 1]))
